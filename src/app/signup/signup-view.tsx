...
export function SignupView({ initialRole = "eater" }: SignupViewProps) {
  const router = useRouter();
  const normalizedRole = (initialRole ?? "eater").toLowerCase();

  const message = "Create an account so you can start!";
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(extractErrorMessage(body, "Unable to register"));
      }
    ...
    <main className="relative min-h-full overflow-hidden bg-white">
      ...
      <div className="relative mx-auto flex min-h-full w-full max-w-sm flex-col px-6 pb-32 pt-16 text-neutral-900">
        <div className="flex items-center justify-between">
          <Link
            href={`/login?role=${normalizedRole}`}
        ...
        <header className="mt-10 text-center">
          <h1 className="text-3xl font-semibold text-[#ff775d]">
            Create Account
          </h1>
        ...
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          ...
          <Button
            ...
            className="mt-2 h-12 w-full rounded-3xl bg-[linear-gradient(90deg,_#f87664,_#ffd67f)] text-base font-semibold text-neutral-900 shadow-[0_16px_30px_rgba(248,137,110,0.35)] transition hover:brightness-105 focus-visible:ring-[#f87664]/40"
          >
        ...
        <div className="mt-8 space-y-4">
          ...
          >
            <Link href={`/login?role=${normalizedRole}`}>Log in</Link>
          </Button>
        </div>
